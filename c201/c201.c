...
void List_DeleteFirst( List *list ) {
	//solved = FALSE; /* V případě řešení, smažte tento řádek! */
    ListElementPtr new_first;
    if((list->firstElement == NULL)||(list == NULL)){
        return;
    }
    else{
        new_first = list->firstElement->nextElement;///ulozeni navaznosti

        if(list->activeElement == list->firstElement){ list->activeElement = NULL; }///pokud byl prvni

        free(list->firstElement); ///zruseni prvniho
        list->firstElement = new_first;///navazani na seznam
    }
}
...
void List_DeleteAfter( List *list ) {
	//solved = FALSE; /* V případě řešení, smažte tento řádek! */
    ListElementPtr toDelete;

    /// seznam existuje         existuje aktivni            seznam neni prazdny
    if((list == NULL)||(list->activeElement == NULL)||(list->firstElement==NULL)){ return;}
    else{
        if(list->activeElement->nextElement == NULL){ ///aktivni je posledni prvek
            return;
        }
        else{
...
void List_InsertAfter( List *list, int data ) {
	//solved = FALSE; /* V případě řešení, smažte tento řádek! */
    ListElementPtr inserted;
    /// list existuje        list ma aktivni        list neni prazdny
    if((list==NULL)||(list->activeElement==NULL)||(list->firstElement == NULL)){
        return;
    }
    inserted = (ListElementPtr) malloc(sizeof (struct ListElement));
    if(inserted == NULL){ List_Error();return;}

    inserted->data = data;
    inserted->nextElement = list->activeElement->nextElement;
    list->activeElement->nextElement = inserted;
...
void List_GetValue( List *list, int *dataPtr ) {
	//solved = FALSE; /* V případě řešení, smažte tento řádek! */
    /// list existuje        list ma aktivni        list neni prazdny
    if((list==NULL)||(list->firstElement==NULL)||(list->activeElement == NULL)){
        List_Error();
        return;
    }
